...
(function(jQuery) {
    ...
    function uSquareItem(element, options) {
        ...
        this.$trigger.on('click', jQuery.proxy(this.show, this));
        this.$close.on('click', jQuery.proxy(this.close, this));
        options.$overlay.on('click', jQuery.proxy(this.close, this));
    };
            ...
            if (!this.$parent.data('in_trans'))
            {
                if (!this.$item.data('showed'))
                {
                    ...
                    if (item_position.top>0) // && this.$parent.width()>=640)
                    {
                        ...
                        var non_visible_area=jQuery(window).scrollTop()-parent_top;
...
//						this_backup.$trigger_text.animate({'top': trigger_text_position_top}, 'slow');
    ...
    function uSquare(element, options) {
        var self = this;
        this.options = jQuery.extend({}, jQuery.fn.uSquare.defaults, options);
        ...
        this.$triggers.on('click', jQuery.proxy(this.overlayShow, this));
        this.$closes.on('click', jQuery.proxy(this.overlayHide, this));
        this.$overlay.on('click', jQuery.proxy(this.overlayHide, this));

        jQuery.each( this.$items, function(i, element) {
            new uSquareItem(element, jQuery.extend(self.options, {$overlay: self.$overlay, $parent: self.$element }) );
        });
    };
    ...
    jQuery.fn.uSquare = function ( option ) {
    ...
    jQuery.fn.uSquare.Constructor = uSquare;
    jQuery.fn.uSquare.defaults = {
        block:     '.usquare_block',
...
jQuery(window).load(function() {
    jQuery(".usquare_about").mCustomScrollbar();
});

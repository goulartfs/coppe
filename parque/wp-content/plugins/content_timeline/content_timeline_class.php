	...
	function ajax_frontend_get(){
		...
		if($timelineId) {
			...
			<div class="timeline_open_content'.(!array_key_exists('desable-scroll', $arr) ? ' scrollable-content' : '').'" style="height: '. $open_content_height.'px">';
					...
					$frontHtml .= '
			<div class="timeline_open_content'.(!isset($arr['desable-scroll']) ? ' scrollable-content' : '').'" style="height: '. (intval($settings['item-height']) - 2*intval($settings['item-open-content-padding'])).'px">';
				}
